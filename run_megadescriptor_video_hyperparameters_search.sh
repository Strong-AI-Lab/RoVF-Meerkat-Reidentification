...
# Script to run hyperparameter search for MegaDescriptor video model
...
YAML_DIR="$BASE_DIR/training_scripts/exp_metadata/hyperparameter_search/megadescriptor_video"
...
echo "Starting MegaDescriptor video hyperparameter search..."
...
for yaml_file in "$YAML_DIR"/*.yml; do
    ...
    CUDA_VISIBLE_DEVICES=4 python main.py train "$yaml_file" -d cuda
...
echo "MegaDescriptor video hyperparameter search completed!"
