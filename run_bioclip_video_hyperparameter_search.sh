...
# Script to run hyperparameter search for BioCLIP video model
...
YAML_DIR="$BASE_DIR/training_scripts/exp_metadata/hyperparameter_search/bioclip_video"
...
echo "Starting BioCLIP video hyperparameter search..."
...
for yaml_file in "$YAML_DIR"/*.yml; do
    ...
    CUDA_VISIBLE_DEVICES=5 python main.py train "$yaml_file" -d cuda
...
echo "BioCLIP video hyperparameter search completed!"
