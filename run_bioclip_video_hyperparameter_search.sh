...
YAML_SUBDIR="training_scripts/exp_metadata/hyperparameter_search/bioclip_video"

usage() {
    cat <<'EOF'
Usage: run_bioclip_video_hyperparameter_search.sh [options]

Run all generated BioCLIP video hyperparameter YAML files.

Options:
  --base-dir DIR        Repository root. Defaults to the current checkout.
  --yaml-dir DIR        YAML directory. Defaults to the BioCLIP video generated YAML directory.
  ...
  --gpu GPU             CUDA_VISIBLE_DEVICES value. Defaults to GPU_DEVICE or 5.
...
rovf_run_yaml_directory_training "BioCLIP video" "$BASE_DIR" "$YAML_DIR" "$PYTHON_BIN" "$DEVICE" "$GPU_DEVICE"
