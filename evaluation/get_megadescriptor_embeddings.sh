...
usage() {
    cat <<'EOF'
Usage: get_megadescriptor_embeddings.sh [options]

Generate MegaDescriptor pre-trained embeddings for the configured datasets.
...
generate_name() {
    ...
    model_size=$(printf '%s\n' "$model" | sed 's/.*MegaDescriptor-\([LBST]\)-.*/\1/')
    printf 'megadescriptor_%s_%s_%sframes_%s_%s\n' "$model_size" "$strat" "$frames" "$dataset" "$mask_option"
}
...
run_embedding() {
    ...
    output_dir="$BASE_DIR/results/pre_trained_model/megadescriptor"
...
printf 'MegaDescriptor embedding generation complete!\n'
