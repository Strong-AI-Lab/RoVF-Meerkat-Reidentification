...
for cp in "${checkpoints[@]}"; do
    ...
    python main.py test '' -cp "$cp" -df "$dataset_file" -d "$device" -m "$mask_file" -cd "$clips_directory" -co "$cooccurrences_file" -imv "True"
    echo "Running without mask for checkpoint: $cp"
    python main.py test '' -cp "$cp" -df "$dataset_file" -d "$device" -cd "$clips_directory" -co "$cooccurrences_file" -imv "True"
done
