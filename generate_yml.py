...
def create_yaml_config_RoVF_S_st_bioclip(
    mask_type=["mask", "no_mask", "50/50"],
    fps=[0.5, 1], 
    aug=["no_aug", "aug"],
): 

    # RoVF_S_st with BioCLIP image model

    filepath = "training_scripts/exp_metadata/hyperparameter_search/RoVF_S_st_bioclip/"
    if not os.path.exists(filepath):
        os.makedirs(filepath)
    ...
    config_orig = {
        'model_details': {
            "model_type": "recurrent_perceiverv2",
            "raw_input_dim": None,
            "embedding_dim": 768,  # BioCLIP transformer token width
            "latent_dim": 384,
            "num_heads": 8,
            "num_latents": 257,  # keep RoVF latent seq length unchanged
            "num_tf_layers": 2,
            "dropout_rate": 0.1,
            "output_dim": 384,
            "use_raw_input": False,
            "use_embeddings": True,
            "flatten_channels": False,
            "dino_model_name": "hf-hub:imageomics/bioclip",
            "freeze_image_model": True,
            "is_append_avg_emb": True
        },
        'dataloader_details': {
    ...
    for mask in mask_type:
        for f in fps:
            for aug_type in aug:
                ...
                config['training_details']['log_directory'] = f"results/hyperparameter_search/RoVF_S_st_bioclip/{mask_filename}/{fps_filename}/{aug_filename}/"

                directory = f"results/hyperparameter_search/RoVF_S_st_bioclip/{mask_filename}/{fps_filename}/{aug_filename}/"
                if not os.path.exists(directory):
                    os.makedirs(directory)
...
def create_yaml_config_bioclip_video(
    mask_type=["mask", "no_mask", "50/50"],
    fps=[0.5, 1],
    aug=["no_aug", "aug"],
):
    filepath = "training_scripts/exp_metadata/hyperparameter_search/bioclip_video/"
    if not os.path.exists(filepath):
        os.makedirs(filepath)
    ...
    for mask in mask_type:
        for f in fps:
            for aug_type in aug:
                ...
                config['training_details']['log_directory'] = f"results/hyperparameter_search/bioclip_video/{mask_filename}/{fps_filename}/{aug_filename}/"

                # Create directory
                directory = f"results/hyperparameter_search/bioclip_video/{mask_filename}/{fps_filename}/{aug_filename}/"
                if not os.path.exists(directory):
                    os.makedirs(directory)
...
def create_yaml_config_megadescriptor_video(
    mask_type=["mask", "no_mask", "50/50"],
    fps=[0.5, 1],
    aug=["no_aug", "aug"],
):
    filepath = "training_scripts/exp_metadata/hyperparameter_search/megadescriptor_video/"
    if not os.path.exists(filepath):
        os.makedirs(filepath)
    ...
    for mask in mask_type:
        for f in fps:
            for aug_type in aug:
                ...
                config['training_details']['log_directory'] = f"results/hyperparameter_search/megadescriptor_video/{mask_filename}/{fps_filename}/{aug_filename}/"

                # Create directory
                directory = f"results/hyperparameter_search/megadescriptor_video/{mask_filename}/{fps_filename}/{aug_filename}/"
                if not os.path.exists(directory):
                    os.makedirs(directory)
